...
description: 发表级学术报告生成器：文献综述 / 课程论文 / 实验报告 / 博士硕士学位论文，从结构化写作到一键导出规范 Word（.docx）。含可运行 Python 脚本——build_academic_docx.py 产出中文学术规范排版（A4/学位论文封面栏含学校专业学号导师日期/中英文摘要与关键词/每章另起一页/首行缩进/三线表/图表编号题注/真·脚注角注/悬挂缩进参考文献/正文引用可点击跳转/页眉+页脚页码，遵 GB/T 7713.1、GB/T 7714），make_figure.py 出 300dpi 发表级插图（图表规则移植自 figure-style：结论式标题、焦点色主导、小样本点+中位数、折线末端直标、渲染自检）。用户要「写文献综述」「课程论文」「实验报告」「学位/毕业论文」「生成学术报告 Word」「把材料整理成规范论文」时用。帮你把真实材料组织成规范初稿并导出成品，不代查文献、不编造数据。
---
...
python3 scripts/build_academic_docx.py report.md -o report.docx \
    --title "标题" --subtitle "副标题" --author "姓名" \
    --header "论文标题" --page-numbers --line-spacing 1.5 --render

# 学位论文完整封面（各栏可选，给了才显示）：
python3 scripts/build_academic_docx.py thesis.md -o thesis.docx \
    --title "论文题目" --school "某某大学" --degree "硕士学位论文" \
    --author "张三" --student-id "2021xxxx" --major "计算机科学与技术" \
    --advisor "李教授" --date "2026 年 7 月" \
    --header "论文题目" --page-numbers --render
```

自动套：**学位论文封面**（学校/论文类型/标题/作者/学号/专业/指导教师/完成日期，标签对齐、单页）、
中/英文摘要**各独占一页**且居中、**关键词顶格**、**每一章另起一页**、
正文宋体小四·1.5 倍行距·**首行缩进 2 字符**、标题黑体分级、
数据表→**三线表**、图/表**居中编号题注**、`[^n]`→**真·脚注**、参考文献**悬挂缩进**、
正文 `[n]` 引用→**可点击跳转**到对应参考文献（内部书签超链接，黑色不变蓝）、
`--header` 加**页眉**（标题+下划线）、`--page-numbers` 加**页脚页码**（均封面首页不显示）。
